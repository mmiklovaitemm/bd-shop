...
const BASE_PATH = process.env.FRONTEND_BASE_PATH || "/";
...
const withBase = (path) => {
  // BASE_PATH: "/" arba "/bd-shop/"
  const base = String(BASE_PATH).replace(/^\/?/, "/").replace(/\/?$/, "/");
...
const DEFAULT_DETAILS_BY_CATEGORY = {
  rings: { bandWidthMm: 25, weightG: 4.5 },
  earrings: { dimensions: { heightMm: 25, widthMm: 16.4 }, weightG: 4.5 },
  necklaces: {
    chain: "Curb",
    totalLengthCm: 45,
    adjustableFromCm: 41,
